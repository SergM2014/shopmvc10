...
window.SwipeListener = __webpack_require__(/*! swipe-listener */ "./node_modules/swipe-listener/dist/swipe-listener.min.js");
/**
...
window.tree = function () {
  return {
    ...
    openChild: function openChild(level, parentId, currentElemId) {
      //set minimised tree marker to false
      this.minimised = false;
      var actualEl = this.$refs.tree.querySelector('[data-id = "' + currentElemId + '"]');

      this._closeUselessGroups(level, parentId);

      var nextLevel = level + 1;
      var childrenGroup = document.querySelector('[data-level = "' + nextLevel + '"]');
      if (!childrenGroup) return; //console.log(childrenGroup);
      ...
      var myItemsParentIdArr = this.$refs.tree.querySelectorAll('.origin');
      var myItemsParentId = false;
      myItemsParentIdArr.forEach(function (item) {
      ...
      var actualOffsetTop = actualEl.offsetTop;
      var actualOffsetHeight = this.$refs.tree.offsetHeight;
      ...
      if (actualOffsetHeight < childrenGroup.offsetHeight) {
        var diff = childrenGroup.offsetHeight - actualOffsetHeight;
        actualOffsetTop = actualOffsetTop - diff;
        if (actualOffsetTop < 0) actualOffsetTop = 0;
        childrenGroup.style.paddingTop = actualOffsetTop + "px";
      }
    },
    _closeUselessGroups: function _closeUselessGroups(level) {
      var branches = this.$refs.tree.querySelectorAll('[data-level]');
      branches.forEach(function (branch) {
        if (branch.dataset.level > level) {
    ...
    closeAll: function closeAll() {
      ...
      var branches = tree.querySelectorAll('[data-level]');
      branches.forEach(function (branch) {
        if (branch.dataset.level > 0) branch.classList.add('hidden');
    ...
    _estRowsDirection: function _estRowsDirection() {
      if (window.innerWidth <= 768) return;
      var actGroupWidth = this.childGroups[0].clientWidth;
      var width = document.getElementById('tree').parentElement.clientWidth;
      var chunk_size = Math.floor(width / actGroupWidth) - 1; //*******************
      ...
      var leftArrRev = [].concat(leftArr).reverse(); //*****************

      var arr = Array.from(this.childGroups); //for technical needs remove first elem of array

      var firstElem = arr.shift(); //console.table(arr)
    ...
    _showSideArrows: function _showSideArrows() {
      //show appropriate arrows
      var arrow = this.childGroups[0].querySelectorAll('.arrow-right');
      arrow.forEach(function (item) {
        return item.classList.remove('hidden');
      });

      for (var i = 0; i < this.directionArr[0].length; i++) {
        for (var i2 = 0; i2 < this.directionArr[0][i].length; i2++) {
          if (i2 === this.directionArr[0][i].length - 1) {
            var _arrow = this.directionArr[0][i][i2].querySelectorAll('.arrow-left');

            _arrow.forEach(function (item) {
              item.classList.remove('hidden');
              item.parentNode.prepend(item);
            });
          } else {
            var _arrow2 = this.directionArr[0][i][i2].querySelectorAll('.arrow-right');

            _arrow2.forEach(function (item) {
              return item.classList.remove('hidden');
            });
          }
        }
    ...
    _showDownArrows: function _showDownArrows() {
      for (var i = 0; i < this.childGroups.length; i++) {
        var arrows = this.childGroups[i].querySelectorAll('.arrow-down');
        arrows.forEach(function (item) {
          return item.classList.remove('hidden');
        });
      }
    },
    _hideDownArrows: function _hideDownArrows() {
      for (var i = 0; i < this.childGroups.length; i++) {
        var arrows = this.childGroups[i].querySelectorAll('.arrow-down');
        arrows.forEach(function (item) {
          return item.classList.add('hidden');
    ...
    _setFirstColumnColor: function _setFirstColumnColor() {
      var _this = this;

      //set parens column color
      var stuffs = this.childGroups[0].querySelectorAll('.stuff');
      stuffs.forEach(function (item) {
        return item.classList.add(_this.directionsColors[0]);
      });
    },
    _paintStuff: function _paintStuff(actualEl, childrenGroup) {
      var _this2 = this;

      //painting each next child items
      var classes = actualEl.classList;
      var color;
      ...
      var stuff = childrenGroup.querySelectorAll('.stuff');
      stuff.forEach(function (item) {
        return item.classList.add(color);
      }); //returning previous color for the last direction stuff in case of turnig direction reverse

      if (actualEl.classList.contains('last') && !actualEl.querySelector('.arrow-left') && !actualEl.querySelector('.arrow-right')) {
        this._getLastStuffColor(actualEl);
      }

      var turningLeft = actualEl.querySelector('.arrow-left:not(.hidden)');

      if (turningLeft) {
        var outgoingParentId = actualEl.closest('.parentIdItems').dataset.parentId;
        var outgoingParentEl = this.$refs.tree.querySelector('[data-id = "' + outgoingParentId + '"]');
        var findPrevieousRight = outgoingParentEl.querySelector('.arrow-right:not(.hidden)');

        if (turningLeft && findPrevieousRight) {
          var children = childrenGroup.querySelectorAll('.stuff');
          children.forEach(function (item) {
            item.classList.add(_this2.directionsColors[1]);
            item.classList.remove(_this2.directionsColors[0]);
          });
          actualEl.closest('.parentIdItems').querySelectorAll('.stuff').forEach(function (item) {
            item.classList.add(_this2.directionsColors[1], 'last');
            item.classList.remove(_this2.directionsColors[0]);
          });
        }
      }

      var turningRight = actualEl.querySelector('.arrow-right:not(.hidden)');

      if (turningRight) {
        var parentDiv = actualEl.closest('.parentIdItems');
        if (!parentDiv) return;
        var _outgoingParentId = parentDiv.dataset.parentId;

        var _outgoingParentEl = this.$refs.tree.querySelector('[data-id = "' + _outgoingParentId + '"]');

        if (!_outgoingParentEl) {
          return;
        }

        var findPrevieousLeft = _outgoingParentEl.querySelector('.arrow-left:not(.hidden)');

        if (turningRight && findPrevieousLeft) {
          var _children = childrenGroup.querySelectorAll('.stuff');

          _children.forEach(function (item) {
            item.classList.add(_this2.directionsColors[2]);
            item.classList.remove(_this2.directionsColors[1]);
          });

          actualEl.closest('.parentIdItems').querySelectorAll('.stuff').forEach(function (item) {
            item.classList.add(_this2.directionsColors[2], 'last');
            item.classList.remove(_this2.directionsColors[1]);
          });
        }
      }
    },
    _getLastStuffColor: function _getLastStuffColor(actualEl) {
      var parentEl = actualEl.closest('.childrenGroup');
      var parentElClasses = parentEl.querySelector('.stuff').classList;
      var currentColorToDel;

      for (var i = 0; i < parentElClasses.length; i++) {
    ...
    _openSmScreenChildren: function _openSmScreenChildren(actualEl, childrenGroup, parentId) {
      var myItemsParentIdArr = this.$refs.tree.querySelectorAll('.origin');
      var myItemsParentId = false;
      myItemsParentIdArr.forEach(function (item) {
        if (item.dataset.parentId == parentId) {
      ...
      var offsetLevels = ['.parentIdItems', '.childrenGroup', '#tree'];
      var offsetTop = actualEl.offsetTop;

      for (var i = 0; i < offsetLevels.length; i++) {
        offsetTop += actualEl.closest(offsetLevels[i]).offsetTop;
      }

      var actualHeight = actualEl.offsetHeight;
      actualOffsetTop = offsetTop + actualHeight;
      childrenGroup.style.top = actualOffsetTop + "px";
    },
    _paintSmallScreenChild: function _paintSmallScreenChild(actualEl, childrenGroup) {
      var parentEl = actualEl.closest('.childrenGroup');
      var parentElClasses = parentEl.querySelector('.stuff').classList;
      var currentColorToDel;
